...
const lambdaDirectory = join(outputDirectory, 'publication');
const bundlePath = join(lambdaDirectory, 'index.mjs');
const zipPath = join(outputDirectory, 'publication.zip');
...
await build({
  entryPoints: ['src/api/publication.mjs'],
  outfile: bundlePath,
  bundle: true,
  platform: 'node',
  target: 'node24',
...
const metadata = {
  path: 'build/publication.zip',
  sha256: createHash('sha256').update(artifact).digest('hex'),
  bytes: artifact.length,
};
await writeFile(join(outputDirectory, 'publication.json'), JSON.stringify(metadata, null, 2) + '\n');
